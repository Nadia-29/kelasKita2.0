...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $transaksi = $query->with(['user', 'metodePembayaran'])->get();

        $metodePembayaranUnik = Transaksi::join('metode_pembayaran', 'transaksi.id_mp', '=', 'metode_pembayaran.id_mp')
            ->select('metode_pembayaran.nama_metode')
            ->distinct()
            ->pluck('nama_metode');
